...
- Currently loads events and assets in a SQLite database
- Should instead store assets/events as graph nodes and edges? Or no?

Hardcopy

- Currently loads graph data in a SQLite database
- Handles syncing to/from
- Originally thought to add hooks/events
- Instead -> Just have skills/Patchwork combine to handle fetching data?

Patchwork

- Should really _only_ rely on MCP data, right?
...
- Relationships?
- 3rd party sources? Call only via Patchwork widgets?
