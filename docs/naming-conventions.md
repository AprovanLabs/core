...
All published packages live under the `@aprovan/` npm scope (GitHub Packages) and are prefixed by their **project name** (`registry-`, `patchwork-`, …). Packages in the `core` repo drop the project prefix — core is the umbrella (`@aprovan/ui`, `@aprovan/main`, `@aprovan/devtools`).
...
| `main` | Shared resources that don't have another name (clients, contracts, shared config loaders) | `@aprovan/registry-main`, `@aprovan/main` (core) |
...
- Environments default to `prd`. Shared runtime config is published as a single SSM parameter per environment in dotenv format: `/aprovan/<env>/env` (see `@aprovan/main` loader).
