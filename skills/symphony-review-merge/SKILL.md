...
**Branch naming convention:** All working branches must follow `<IDENTIFIER>/<short-slug>` format (e.g., `APR-47/fix-login`). This is enforced by `ci-validation` §1.0 before any PR is opened. When rebasing or creating any new commits, confirm the branch name is still correct with `git branch --show-current`.
...
After rebasing, verify the branch name is still correct (`git branch --show-current` should still match `^[A-Z]+-\d+/`), then confirm CI passes on the rebased state using the `github` MCP `get_pull_request_status` tool (`owner: AprovanLabs`, `repo: <repo>`, `pull_number: <n>`). Wait for all checks to show `success`.
